  ...
  const handleSubmit = async (e) => {
    try {
      ...
      await handleCreatePO({
        variables: { input: formData, access_token: localStorage.getItem("access_token") },
        refetchQueries: [{ query: GET_PO }],
      });
      history.push("/main");
    } catch (err) {
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Form.Row>
          <Form.Group className="col-md-6">
            <Form.Label>
              <h5>
                <i className="fa fa-calendar" /> Expired Date
              </h5>
            </Form.Label>
