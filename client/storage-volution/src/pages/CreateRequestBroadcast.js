  ...
  const handleSubmitBroadcast = async () => {
    try {
      ...
      await submitBroadcast({
        variables: { itemsToPick: formInput, idStoreReq: id, access_token: localStorage.getItem("access_token") },
    ...
    <>
      <div>
        ...
        <div>
            ...
            return (
              <>
                <Form.Group key={index} className="col">
                  <Form.Label className="my-3" style={{ textTransform: "capitalize" }}>
                    <h5>
                      <i className="fa fa-box" /> {item.itemName} - {item.quantityRequest} Box
                    </h5>
                  </Form.Label>
                  {formInput[index] &&
                        ...
                        <>
                          <Form.Row key={`${idx}a${index}`}>
                            <Form.Group className="col-md-3">
                              <Form.Label>
                                <h5>
                                  <i className="fa fa-user" /> PO ID
                                </h5>
                              </Form.Label>
                              <Form.Control readOnly value={po.idPO}></Form.Control>
                            </Form.Group>
                            <Form.Group className="col-md-3">
                              <Form.Label>
                                <h5>
                                  <i className="fa fa-user" /> Quantity (Box)
                                </h5>
                              </Form.Label>
                        ...
                        <Form.Row>
                                ...
                                if (item.itemName === dropdown.name) {
                                  return dropdown.PO.map((dropdownItem) => {
                                    if (countAddedPO(index) < item.quantityRequest) {
                                      return (
                                        <option key={dropdownItem._id} value={dropdownItem._id}>
                                          {dropdownItem._id}
                                        </option>
                                      );
                                    }
                          ...
                          <div className="col-md-3">
                            <MDBInput
                              ...
                              min="0"
                              max={
                                itemsPOTemp[index].selected
                                  ? item.quantityRequest - countAddedPO(index) < itemsPOTemp[index].current_quantity
                                    ? itemsPOTemp[index].current_quantity
                                    : item.quantityRequest - countAddedPO(index)
                                  : item.quantityRequest - countAddedPO(index)
                              }
                              id={`quantity_${index}`}
                              type="number"
                          ...
                          <Form.Group className="col-md-3">
                            <br />
                            <Button disabled={itemsPOTemp[index].selected ? false : true} type="submit" className="mt-2">
                              +
                            </Button>
                          </Form.Group>
                        </Form.Row>
          ...
          <Button onClick={handleSubmitBroadcast} className="mt-4">
            Submit Broadcast
          </Button>
        </div>
      </div>
